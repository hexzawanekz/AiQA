...
# Paths: reuse Auto-Report's views, assets, reports
_BASE = Path(__file__).resolve().parent
_AUTO_REPORT = _BASE.parent / "Auto-Report"
PROJECT_ROOT = _AUTO_REPORT
VIEWS_PATH = _AUTO_REPORT / "views" / "html"
REPORTS_DIR = _AUTO_REPORT / "reports"
EXPORTS_DIR = REPORTS_DIR / "exports"
TEMPLATE_PATH = REPORTS_DIR / "templates" / "template_temp.docx"
...
def api_project_get(pid):
    ...
    data["assigned_test_case_ids"] = [r["id"] for r in assigned]
    return jsonify(data)
...
def api_last_run_summary():
    ...
    with get_connection() as conn:
        ...
        if not row or row["run_id"] is None:
            return jsonify({"error": "No test run found"}), 404
        run_id = row["run_id"]
...
def api_chat_status():
    ...
    port = os.environ.get("AGENTCHATTR_PORT", "8300")
    try:
        req = urllib.request.urlopen(f"http://127.0.0.1:{port}/", timeout=3)
        available = req.status == 200
    except Exception:
        available = False

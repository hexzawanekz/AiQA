...
app = FastAPI(title="AiQA Portal")

# AiQA QA-specific routes (test plan upload, run, status, adhoc)
try:
    from aiqa_routes import router as aiqa_router
    app.include_router(aiqa_router)
except Exception as e:
    log.warning("AiQA routes not loaded: %s", e)
...
# --- Optional web password (set by configure(); env var name in config [server] auth_password_env) ---
auth_password: str = ""
auth_sessions: dict[str, float] = {}  # cookie_value -> expiry timestamp
AUTH_COOKIE_NAME = "agentchattr_web_auth"
AUTH_SESSION_DAYS = 7
...
def _install_security_middleware(token: str, cfg: dict):
    ...
    # Optional web password: only holders of a valid auth cookie can load / and get the session token
    auth_env = cfg.get("server", {}).get("auth_password_env", "")
    _self.auth_password = (os.environ.get(auth_env, "") or "").strip()
    ...
    class SecurityMiddleware(BaseHTTPMiddleware):
        async def dispatch(self, request: Request, call_next):
            ...
            # Login, logout, favicon: allow without session token (normalize path for trailing slash)
            path_stripped = path.rstrip("/") or "/"
            if path_stripped in ("/login", "/auth/logout") or path == "/favicon.ico":
                return await call_next(request)

            # If web password is set, "/" requires a valid auth cookie
            if path == "/" and _self.auth_password:
                cookie_val = request.cookies.get(_self.AUTH_COOKIE_NAME, "")
                now = time.time()
                if not cookie_val or _self.auth_sessions.get(cookie_val, 0) <= now:
            ...
            if path in ("/", "/qa") or path.startswith(("/static/", "/uploads/", "/api/roles", "/api/aiqa/")):
                return await call_next(request)
            ...
            # MCP proxy: some CLIs send POST /mcp to the web port (8300); forward to MCP server (8200).
            if (path == "/mcp" or path.startswith("/mcp/")) and request.method in ("POST", "GET"):
                client_ip = request.client.host if request.client else ""
                if client_ip in ("127.0.0.1", "::1", "localhost"):
                    return await call_next(request)
                # Non-loopback to /mcp on web port → 403 (MCP is loopback-only)
                return JSONResponse({"error": "MCP is only available on loopback"}, status_code=403)

            # --- Origin check (blocks cross-origin / DNS-rebinding attacks) ---
            # When behind nginx, browser sends Origin like http://YOUR_PUBLIC_IP; allow same-host.
            origin = request.headers.get("origin")
            if origin and origin not in allowed_origins:
                host = request.headers.get("host", "").split(":")[0]
                if host and (origin == f"http://{host}" or origin == f"https://{host}" or
                             origin == f"http://{host}:80" or origin == f"https://{host}:443"):
                    pass  # same-host origin (e.g. nginx at public IP)
                else:
                    return JSONResponse(
...
# --- Optional web password: login / logout (only active when auth_password_env is set) ---

_LOGIN_HTML = """<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>agentchattr — Sign in</title>
<style>body{font-family:system-ui,sans-serif;max-width:320px;margin:3rem auto;padding:1rem}
input{width:100%;box-sizing:border-box;padding:0.5rem;margin:0.5rem 0}
button{width:100%;padding:0.6rem;margin-top:0.5rem;cursor:pointer}
.error{color:#c00;font-size:0.9rem}</style></head>
<body><h1>agentchattr</h1><p>Enter the room password to continue.</p>
<form method="post" action="/login">
<input type="password" name="password" placeholder="Password" autocomplete="current-password" required autofocus>
<div class="error" id="err"></div>
<button type="submit">Sign in</button>
</form>
<script>const params=new URLSearchParams(location.search);if(params.get('invalid')){document.getElementById('err').textContent='Invalid password.'}</script>
</body></html>
"""
...
async def login_page():
    """Show login form when auth_password is set. Otherwise redirect to /."""
...
async def login_submit(request: Request):
    """Check password and set auth cookie on success."""
    if not auth_password:
        return RedirectResponse(url="/", status_code=302)
    body = await request.body()
    # Support both form (password=...) and JSON ({"password":"..."})
    pw = None
    if request.headers.get("content-type", "").startswith("application/json"):
        ...
        pw = (parsed.get("password", [""])[0] or "").strip()
    if pw != auth_password:
        return RedirectResponse(url="/login?invalid=1", status_code=302)
    cookie_val = secrets.token_urlsafe(32)
    auth_sessions[cookie_val] = time.time() + AUTH_SESSION_DAYS * 86400
...
@app.get("/auth/logout")
async def logout():
    """Clear auth cookie and redirect to login (or / if no password)."""
    response = RedirectResponse(url="/login" if auth_password else "/", status_code=302)
    response.delete_cookie(AUTH_COOKIE_NAME)
    return response


# MCP proxy is registered in run.py so POST /mcp on 8300 is always forwarded to the MCP server (8200).
...
def configure(cfg: dict, session_token: str = ""):
    ...
    config = cfg
    # --- Security: store the session token and install middleware (includes optional auth_password) ---
...
async def _handle_new_message(msg: dict):
    ...
    for target in targets:
        ...
        if not mcp_bridge.is_online(target):
            ...
            continue  # Only trigger online instances — offline slots have no process reading their queue
            ...
            if event.get("type") == "message":
                ...
                if text.startswith("/"):
                    ...
                    if cmd == "/continue":
                        ...
                        continue
                    if cmd == "/test":
                        desc = " ".join(cmd_parts[1:]).strip() if len(cmd_parts) > 1 else ""
                        if not desc:
                            store.add("system", "Usage: /test <description> e.g. /test verify add to cart works", msg_type="system", channel=channel)
                        else:
                            try:
                                from aiqa_service import run_adhoc, get_default_client
                                client = get_default_client()
                                result = await run_adhoc(client, desc)
                                run_id = result.get("run_id", "")
                                store.add("system", f"AiQA ad-hoc test started (run_id={run_id}). Check /api/aiqa/status/{run_id} for progress.", msg_type="system", channel=channel)
                            except Exception as e:
                                store.add("system", f"AiQA ad-hoc test failed: {e}", msg_type="system", channel=channel)
                        continue

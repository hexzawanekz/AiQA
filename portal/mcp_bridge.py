...
def _request_from_ctx(ctx: Context | None):
    """Get the HTTP request from MCP context, or None."""
    if ctx is None:
        return None
    try:
...
def _is_loopback(ctx: Context | None) -> bool:
    """True if the MCP request is from loopback (localhost)."""
    request = _request_from_ctx(ctx)
    ...
    *,
    ...
    if registry:
        ...
        if registry.is_agent_family(provided):
            # Allow loopback callers with registered sender (CLIs that don't send Bearer token)
            if _is_loopback(ctx) and registry.is_registered(provided):
                pass
            else:
...
def chat_send(
    ...
    log.info("chat_send called raw_sender=%r message_len=%d channel=%s", sender, len(message or ""), channel)
    sender, err = _resolve_tool_identity(sender, ctx, field_name="sender", required=True)
    if err:
        log.warning("chat_send identity rejected: %s", err)
        return err
    ...
    channel = channel.lstrip("#") if channel else "general"
    ...
    with _presence_lock:
        _presence[sender] = time.time()
    log.info("chat_send OK sender=%s msg_id=%s", sender, msg.get("id"))
...
def chat_read(
    ...
    log.info("chat_read called sender=%r channel=%r job_id=%s", sender, channel or "(all)", job_id)

...
class RuntimeRegistry:
    ...
    def resolve_to_instances(self, name: str) -> list[str]:
        ...
        If `name` is a base family name, returns all active instances in that
        family (e.g. @kilo → [kilo, kilo-1] so every running Kilo gets the trigger).
        If `name` is only an instance name (not a base), returns [name].
        Otherwise returns [name] unchanged (for non-agent names like 'ben').
        """
        with self._lock:
            # Base name: trigger all active instances so @kilo reaches every running Kilo

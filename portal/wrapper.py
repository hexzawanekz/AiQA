...
def _write_kilo_mcp_settings(config_file: Path, url: str, *, token: str = "") -> Path:
    """Write/merge Kilo CLI kilo.json with MCP under 'mcp' key, type 'remote'.

    Kilo rejects unknown keys (e.g. mcpServers, security from Gemini-style config).
    We write only the 'mcp' key; merge with existing mcp section if present.
    """
    ...
    config_file.write_text(json.dumps({"mcp": servers}, indent=2) + "\n", "utf-8")
    return config_file
...
def _write_kimi_mcp_config(
    config_file: Path,
    url: str,
    *,
    token: str = "",
) -> Path:
    """Write Kimi CLI MCP config file. Kimi uses 'transport' key (not 'type')."""
    config_file.parent.mkdir(parents=True, exist_ok=True)
    existing: dict = {}
    if config_file.exists():
        try:
...
_BUILTIN_DEFAULTS: dict[str, dict] = {
    ...
    "kimi": {
        ...
        "mcp_transport": "http",
        "mcp_config_format": "kimi",  # kimi uses "transport" key, not "type"
    },
}
...
def _apply_mcp_inject(
    ...
    *,
    ...
    if mode == "settings_file":
        # Write a settings JSON file at a user-specified path (e.g. .qwen/settings.json, .kilo/kilo.json)
        ...
        if not target.is_absolute():
            ...
            target = base / target
        # Kilo CLI uses "mcp" key with type "remote"; others use "mcpServers"
        if "kilo" in raw_path.lower():
            settings_path = _write_kilo_mcp_settings(target, server_url, token=token)
        else:
        ...
        fmt = inject_cfg.get("mcp_config_format", "claude")
        if fmt == "kimi":
            settings_path = _write_kimi_mcp_config(
    ...
    while True:
        try:
            ...
            if queue_file.exists() and queue_file.stat().st_size > 0:
                ...
                if has_trigger:
                    # Log trigger for debugging (e.g. why Kilo doesn't respond)
                    try:
                        _cur_name, _q = get_identity_fn()
                        _log_path = ROOT / "data" / f"trigger_{_cur_name}.log"
                        ROOT.joinpath("data").mkdir(parents=True, exist_ok=True)
                        with open(_log_path, "a", encoding="utf-8") as _lf:
                            import time as _tt
                            _lf.write(f"[{_tt.strftime('%H:%M:%S')}] trigger channel={channel} lines={len(lines)}\n")
                    except Exception:
                        pass
                    ...
                    # Use current identity (may have changed via rename) for prompt and MCP hint
                    current_name, _ = get_identity_fn()
                    # Lead with chat_send requirement so the agent never replies only in terminal (Gemini etc.)
                    mcp_instruction = (
                        f"You must reply in chat using MCP: call chat_read(channel={repr(channel)}), "
                        f"then chat_send(sender={repr(current_name)}, message='your reply', channel={repr(channel)}). "
                        "Do not respond only in terminal.\n\n"
                    )
                    if custom_prompt:
                        prompt = mcp_instruction + custom_prompt
                    elif job_id:
                        prompt = mcp_instruction + f"mcp read job_id={job_id} - you were mentioned in a job thread, take appropriate action"
                    else:
                        prompt = mcp_instruction + f"mcp read #{channel} - you were mentioned, take appropriate action"
                    ...
                    try:
                        ...
                        with open(_log_path, "a", encoding="utf-8") as _lf:
                            import time as _tt
                            _lf.write(f"[{_tt.strftime('%H:%M:%S')}] inject prompt_len={len(prompt)}\n")
                    except Exception:
                        pass
                    inject_fn(prompt)
            ...
            try:
                ...
                with open(_log_path, "a", encoding="utf-8") as _lf:
                    import time as _tt
                    _lf.write(f"[{_tt.strftime('%H:%M:%S')}] queue_watcher ERROR: {e}\n")
            except Exception:
                pass
